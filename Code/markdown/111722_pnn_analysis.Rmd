...
```{r loading packages}
pacman::p_load(ComplexHeatmap, cowplot, circlize,
      ...
      for (c in unique(current_cohort$condition)) {
        ...
        for (cor in unique(current_coordinate$coordinates)) {
          ...
          for (r in 1:nrow(intensity_sample)) {
            ...
            write.csv(res_df, paste0("Outputs/pnn_analysis/density_data/", c, "_", h, "_", co, "_", s, "_", cor, "_111722.csv"))
          }
        }
      }
    }
...
for (s in unique(all_plot_data$subregion)) {
  ...
  if (s == "MOs") {
    p1 <- Heatmap(matrix = lower_estimates, heatmap_legend_param = list(title = "Difference in Means\n(x-y)"), cluster_rows = FALSE, cluster_columns = FALSE, row_names_side = "left", column_names_centered = TRUE, column_names_rot = 0, row_order = c("SH P0-P5", "SH P0-P1", "SW P0-P5", "SW P0-P1", "NH", "NW"), na_col = "white", row_names_gp = gpar(fontsize = 5, fontface = "bold"), column_names_gp = gpar(fontsize = 5, fontface = "bold"), column_title = paste0(s, " | Mean 2"), cell_fun = function(j, i, x, y, width, height, fill) {
      if (my_t_tests[[paste0(colnames(test_mat)[j], "_", rownames(test_mat)[i])]][17] < 0.05) {
        grid.text(sprintf("%s", my_t_tests[[paste0(colnames(test_mat)[j], "_", rownames(test_mat)[i])]][17]), x, y, gp = gpar(fontsize = 10))
      }
  ...
  png(file = paste0("Outputs/pnn_analysis/heatmaps/subregion_", s, "_complex_heatmap.png"), units = "in", width = 5, height = 0.3937 * 6 + 0.7423, res = 600)
...
save_plot(filename = "Outputs/pnn_analysis/heatmaps/combo_plot_complexheatmap.png", plot = combo_plot, ncol = 5, nrow = 5, base_asp = 1.1, bg = "white")
```
