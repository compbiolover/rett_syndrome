...
# For 3D data
euclidean_distance_3d <- function(a, b, c) {
  # Check that all three vectors have the same number of observations
...
counter <- 1
all_imgs <- list()
for (i in unique(mean_neighbor_df$image)) {
  current_img <- filter(mean_neighbor_df, image == i)
  p1 <- ggplot(aes(x = image, y = positive_neighborhood_mean), data = current_img) +
...
overall_title <- "+ Neighborhood Mean"
p2 <- annotate_figure(p2, top = text_grob(overall_title,
  face = "bold", size = 22
))


ggsave(p2, filename = "positive_neighborhood_mean_violin_plot.png", device = "png", path = "Outputs/", width = 16, height = 16, dpi = 600, units = "in", bg = "white")
...
```{r color coding the scatter plots by image}
plots <- list()
counter <- 1
...
for (c in unique(mean_neighbor_df_simplified$condition_spec)) {
  current_cond <- filter(mean_neighbor_df_simplified, condition_spec == c)
  current_cond$image <- as.factor(current_cond$image)
  p1 <- ggscatter(
    data = current_cond,
    x = "mean", y = "positive_neighborhood_mean",
    ...
    cor.coeff.args = list(
      method = "spearman",
      label.x = 2000, label.y = 1700, label.sep = "\n",
      cor.coef.name = "rho"
    ),
    parse = TRUE,
    cor.coef.size = 10,
  ...
  # Change to p2 when in a single annotation context, p5 otherwise
  ggsave(plot = p2, filename = paste0("knn_", k_n, "_", c, "_positive_neighborhood_correlation_analysis_p_cells_only_", stat_used, "_only_p_cells_included_colored_by_image.", file_type_used), path = "Outputs/knn_analysis/plots/correlation_analysis/", device = file_type_used, dpi = 600, width = 8, height = 8, units = "in")
...
ggsave(plot = combo_plot, filename = paste0("knn_", k_n, "_combo_plot_neighborhood_correlation_analysis_p_cells_only_", stat_used, "_only_p_cells_included_with_contours_colored_by_image.", file_type_used), path = "Outputs/knn_analysis/plots/correlation_analysis/", device = file_type_used, dpi = 600, width = 12, height = 12, units = "in", bg = "white")


for (i in 1:length(plots)){
  ggsave(plot = plots[[i]], filename = paste0("knn_", k_n, "_combo_plot_neighborhood_correlation_analysis_p_cells_only_", stat_used, "_only_p_cells_included_no_contours_colored_by_image_",i, ".svg"), path = "Outputs/knn_analysis/plots/correlation_analysis/", device = "svg", dpi = 600, width = 8, height = 8, units = "in", bg = "white" )
}

```


# Doing Mean Intensity by image sina plots
```{r mean intensity by image sina plots}
# Trying a slightly different analysis with doing the same scatter plot stuff as the color coding but for the violin plots
all_imgs <- list()
counter <- 1
mean_neighbor_df_simplified <- mean_neighbor_df
