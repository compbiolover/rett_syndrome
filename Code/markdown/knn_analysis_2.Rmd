...
nearest_neighbors <- function(data, observation, num_neighbors, distance_func, p_val = NULL, apply_intensity = TRUE, intensity_data = my_x, p_only = TRUE, calc_mean_intensity = FALSE, custom_neighbors = NULL, random_num_neighbors = FALSE) {
  ...
  dist <- if (distance_func == "euclidean_distance_3d"){
    euclidean_distance_3d(
      observation$x, observation$y, observation$z,
      data$x, data$y, data$z
    )
  } else {
    euclidean_distance(observation$x, observation$y,
      data$x, data$y)
  }
...
# Trying update 
```{r trying corrected updated 3d code}
...
for (i in 1:length(all_imgs)) {
  ...
  for (r in 1:nrow(current_img)) {
    my_obs <- current_img[r, ]
    my_x <- filter(current_img, id != my_obs$id)
    my_ind <- nearest_neighbors(data = my_x[, 3:5], observation = my_obs[, 3:5], num_neighbors = 3, distance_func = euclidean_distance_3d, apply_intensity = TRUE, p_only = TRUE)[[1]]
...
for (i in 1:length(all_imgs)) {
  ...
  for (r in 1:nrow(current_img)) {
    my_obs <- current_img[r, ]
    my_x <- filter(current_img, id != my_obs$id)
    current_cell_neighbor_mean <- nearest_neighbors(my_x[, 3:5], my_obs[, 3:5], num_neighbors = k_n, distance_func = function(a, b) euclidean_distance_3d(x1 = my_obs$x, y1 = my_obs$y, z1 = my_obs$z), intensity_data = my_x, p_only = TRUE, calc_mean_intensity = TRUE, apply_intensity = TRUE)
    img_means <- c(img_means, current_cell_neighbor_mean)
    current_img[r, "positive_neighborhood_mean"] <- current_cell_neighbor_mean
  }
...
# Doing it with corrected 3D euclidean calculation
```{r 3d data volin plot distribution complete 6WO data corrected 3d euclidean calculation}
threed_data <- read.csv("Data/Old data/Jacob/3D_MECP2_6WO_combined_dataset.csv")
threed_data <- threed_data %>%
  select(Filename, Mean, CX..pix., CY..pix., CZ..pix.) %>%
  rename_all(tolower) %>%
