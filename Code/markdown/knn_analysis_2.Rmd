...
for (k in k_ns) {
  for (i in 1:length(all_imgs)) {
    ...
    for (r in 1:nrow(current_img)) {
      my_obs <- current_img[r, ]
      my_x <- filter(current_img, id != my_obs$id)
      current_cell_neighbor_mean <- nearest_neighbors(my_x[, 7:8], my_obs[, 7:8], k = k, FUN = euclidean_distance, use_intensity = TRUE, use_p_only = TRUE, mean_neighborhood_intensity = TRUE)
      img_means <- c(img_means, current_cell_neighbor_mean)
      current_img[r, "positive_neighborhood_mean"] <- current_cell_neighbor_mean
    }
...
```{r knn random neighbors at k of 5}
k_ns <- seq(1, 15, 1)
file_type_used <- "png"
stat_used <- "spearman"
fit_line <- "reg.line"
...
# Have to set the seed for reproducibility
cor_results <- list()
counter <- 1
set.seed(123)
for (i in 1:length(all_imgs)) {
  current_img <- all_imgs[[i]]
  current_img_sampled <- current_img %>% sample_frac(0.8)
  for (r in 1:nrow(current_img_sampled)) {
    current_img_sampled_neigh <- current_img_sampled %>% sample_n(size = 3)
    current_img_sampled[r, "positive_neighborhood_mean"] <- mean(current_img_sampled_neigh$mean)
  }
  cor_res <- cor.test(x = as.numeric(as.vector(unlist(current_img_sampled$mean))), y = as.numeric(as.vector(unlist(current_img_sampled$positive_neighborhood_mean))), method = "spearman")
  cor_results[[counter]] <- cor_res
  counter <- counter + 1
}
```
...
k_rand_df <- data.frame(
  k = rep(3, times = 8),
  condition = rep(c("NH", "SH"), each = 4)
)

cors <- c()
for (r in 1:length(cor_results)) {
  current_cor <- cor_results[[r]]
  cors <- c(cors, as.numeric(as.vector(unlist(current_cor$estimate))))
}

k_rand_df$rho <- cors
k_rand_df$condition <- c("NH", "SH", "NH", "SH", "SH", "SH", "NH", "NH")

p1 <- ggplot(data = k_rand_df, aes(x = condition, y = rho, color = condition, fill = condition)) +
  ...
  ylab(expression(rho)) +
  ggtitle("K = 3") +
  scale_fill_discrete(type = c("steelblue", "orange")) +
  scale_color_discrete(type = c("steelblue", "orange")) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", linewidth = 1.25)

ggsave(plot = p1, filename = "nh_vs_sh_random_knn_3_vs_rho.svg", device = "svg", width = 8, height = 8, units = "in", path = "Outputs/knn_analysis/plots/")
```

```{r histogram of correlations}
p1 <- ggplot(data = k_rand_df, aes(x = rho)) +
  geom_histogram() +
  theme(
    panel.background = element_blank(),
    axis.title = element_text(size = 18, face = "bold"),
...
ggsave(plot = p1, filename = "nh_vs_sh_random_knn_3_vs_rho_histogram.svg", device = "svg", width = 8, height = 8, units = "in", path = "Outputs/knn_analysis/plots/")
```


```{r another attempt at correlation analysis}
k_n <- 5
file_type_used <- "png"
stat_used <- "spearman"
fit_line <- "reg.line"
...
for (n in 1:1000) {
  ...
  for (i in 1:length(all_imgs)) {
    ...
    write.csv(current_img, paste0("Outputs/knn_analysis/data/1000_simulation/img_", i, "_knn_", k_n, "_df_16_bit_mean_neighborhood_analysis_random_", k_n, "_sample_", n, ".csv"))
    img_means <- c()
  }

  all_dfs <- vector(mode = "list", length = 4)
  for (i in 1:length(all_imgs)) {
    current_df <- read.csv(paste0("Outputs/knn_analysis/data/1000_simulation/img_", i, "_knn_", k_n, "_df_16_bit_mean_neighborhood_analysis_random_", k_n, "_sample_", n, ".csv"))
    all_dfs[[i]] <- current_df
  }
...
# Function to read in all the files and perform all correlation tests
read_files <- function(folder_path, pattern, combine_all_files = FALSE, perform_cor = TRUE) {
  ...
  # Function to read and perform cor.test
  process_file <- function(file) {
    data <- read.csv(file)

    if (perform_cor) {
      cor_res <- cor.test(x = data$mean, y = data$positive_neighborhood_mean, method = "spearman")
      return(list(data = data, cor_res = cor_res))
    } else {
      return(list(data = data))
    }
  }

  # Read in all CSV files and perform cor.test if needed
  file_results <- lapply(file_list, process_file)

  if (combine_all_files) {
    data <- do.call(rbind, lapply(file_results, function(x) x$data))
    return(data)
  } else {
    return(file_results)
  }
...
all_nh_k_5_df <- read_files(folder_path = "Outputs/knn_analysis/data/1000_simulation/", pattern = "*.csv", combine_all_files = FALSE, perform_cor = TRUE)

all_rhos_df <- data.frame(
  k = rep(5, times = 1000),
  condition = rep("NH", times = 1000),
