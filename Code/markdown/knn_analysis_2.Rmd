...
# 3D data violin plot
```{r 3d data volin plot distribution}
...
```{r mean intensity by image sina plots 3d}
# Trying a slightly different analysis with doing the same scatter plot stuff as the color coding but for the violin plots
all_imgs <- list()
counter <- 1
mean_neighbor_df_simplified <- mean_neighbor_df
...
for (c in unique(mean_neighbor_df_simplified$condition)) {
  ...
  p1 <- ggscatter(
    ...
    cor.coeff.args = list(
      method = "spearman",
      label.x = 2500, label.y = 1150, label.sep = "\n",
      cor.coef.name = "rho"
    ),
    parse = TRUE,
    cor.coef.size = 10,
...
for (i in 1:length(plots)){
  ggsave(plot = plots[[i]], filename = paste0("knn_", k_n, "_combo_plot_neighborhood_correlation_analysis_p_cells_only_", stat_used, "_only_p_cells_included_no_contours_colored_by_image_3d",i, ".svg"), path = "Outputs/knn_analysis/plots/correlation_analysis/", device = "svg", dpi = 600, width = 8, height = 8, units = "in", bg = "white" )
}
