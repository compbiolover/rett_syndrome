...
## KNN analysis 
```{r knn functions}
nearest_neighbors <- function(x, obs, k, FUN, p = NULL, use_intensity = TRUE, intensity_df = my_x, use_p_only = TRUE) {
  obs_to_remove <- c()
  ...
  if (use_intensity == TRUE & use_p_only == FALSE) {
    ...
    print(nrow(intensity_info))
    if (nrow(intensity_info) == 0) {
      print("This sample has only N neighbors. It is being excluded from this analysis.")
