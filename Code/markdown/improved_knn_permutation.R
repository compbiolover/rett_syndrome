...
permutation_knn_analysis <- function(all_imgs,
  ...
  for (iteration in 1:num_iterations) {
    ...
    for (i in seq_along(all_imgs)) {
      ...
      # Performing the correlation test between the mean and the positive (mecp2_p = P) neighborhood mean 
      cor_res <- cor.test(
        x = df$mean,
    ...
    results[[iteration]] <- data.frame(rho = iteration_rhos,
                                       p_value = iteration_p_values,
  ...
  print(length(iteration_rhos))
  print(length(iteration_p_values))
...
#' Permutation KNN Analysis
#'
#' Perform k-nearest neighbor analysis for each row in a dataframe using permutation.
#'
#' @import foreach
#' @import parallel
#' @import dplyr
#'
#' @param all_imgs A list of dataframes representing the images in the dataset. The images will all be from the same condition based on pre-processing.
#' @param num_nearest The number of nearest neighbors to consider for picking random neighbors within the local group. Default is 30.
#' @param chosen_cond The condition to filter the dataframes in the `all_imgs` list. Defaults to "NW".
#' @param custom_seed A custom random seed to use for reproducibility. If not provided, defaults to 1689014354. Set to `NULL` to generate a random seed based on the system time.
#' @param k The number of neighbors to consider for k-nearest neighbor analysis. Defaults to 5.
#' @param random_num_neighbors Should we perform random nk-nearest neighbor selection. Default is FALSE
#' @param num_iterations The number of iterations for the permutation analysis. Defaults to 1000.
#' @param num_cores The number of CPU cores to use for parallel computation. Default is `NULL` which detects number of CPU cores and uses all of them except one.
#'
#' @details
#' This function requires the following packages to be installed: `foreach`, `parallel`, and `dplyr`.
#' Use install.packages() to install the packages
#' Please make sure to load these packages before running the function.
#'
#' @return A dataframe with columns for rho (correlation value), p_value (p-value), condition, time point, k, and k_search_space.
#'
#' @examples
...
#' library(dplyr)
#'
#' # Generate the permutation KNN analysis for condition "NW" with k = 5
#' result <- permutation_knn_analysis(all_imgs, num_nearest = 30, chosen_cond = "NW", custom_seed = 1689014354, k = 5, num_iterations = 1000, num_cores = 4)
#' print(result)
#'
#' @export
# permutation_knn_analysis <- function(all_imgs,
...
#     for (i in seq_along(all_imgs)) {
#       df <- all_imgs[[i]]
#       # cat("Dimensions of data frame are: ", dim(df), "\n")
#       n <- nrow(df)
#       
#       all_p_scores <- list()
#       all_data <- list()
...
#           # Then pass the randomly selected indices from the num_nearest neighbors to the other points
#           k_nearest_points <- other_points[k_nearest_indices, ]
#           # cat("Done selecting random neighbors from neighbor set of", num_nearest, "neighbors for this row in current image\n")
#           # cat("Its semi-random nearest points are:\n")
...
#         
#         # Calculating other parts of the score beyond just KNN
#         # add_p_score is the number of neighbors in a neighborhood who are annotated as P divided by the number of
#         # neighbors we are looking for (typically k = 5)
#         add_p_score <- sum((k_nearest_points$mecp2_p == "P")) / k
#         observation$knn_label <- add_p_score
#         all_p_scores[[counter]] <- add_p_score
#         
...
#           # Calculate updated distances with intensity information for just P neighbors
#           if (p_only) {
#             k_nearest_points <- filter(k_nearest_points, mecp2_p == "P")
#             if (nrow(k_nearest_points) == 0) {
#               writeLines("This sample has only N neighbors.\nIt is being excluded from this analysis.")
#               next
#             }
...
#   result_df <- result_df[order(result_df$rho, decreasing = TRUE), ]
#   
#   # Return the result data frame
#   return(result_df)
# }
...
# Test of permutation code
nw_results_10000 <- permutation_knn_analysis(all_imgs,
                                         num_nearest = 30,
...
plot_permutation_test <- function(data, filename, x_intercept, plot_size = 10, base_plot_size = 8, dpi = 600, hist_color = "steelblue",
  ...
  p <- ggplot(data = data, aes(x = rho)) +
    ...
    ggtitle(paste0("K = ", k_value, " | ", unique(data$condition), " | ", num_permutations, " Permutations | ", time_point, " WK | ", num_ks, "KNNs Used" )) +
    xlab(expression(rho)) +
    ylab("Count") +
...
nw_plot <- plot_permutation_test(nw_data, "nw_random_knn_vs_rho_histogram_test", 0.27,
  plot_size = 2.5, base_plot_size = 8, dpi = 600, hist_color = "red",
  hist_fill = "pink", line_color = "blue", text_color = "blue"
)
