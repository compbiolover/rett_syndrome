...
total_plot <- ggplot(aes(x=Combo,y=Mean, fill=Condition), data = mecp2_6_12_pos)+
  geom_violin(colour="white", draw_quantiles = c(0.25, 0.5, 0.75))+ theme(panel.background = element_blank(),
...
total_plot <- ggplot(aes(x=Combo, y=Mean, fill=Condition),
                     data = mecp2_6_12_neg)+
  geom_violin(colour="black", fill="white",draw_quantiles = c(0.25, 0.5, 0.75)) +
  theme(panel.background = element_blank(),
...
total_plot <- ggplot(aes(x=Combo, y=Mean, fill=Condition),
  ...
  geom_signif(y_position = c(2750, 2500), xmin = c(1.0,3.0), xmax = c(2.0,4.0),
              annotations = c("**", "ns"), tip_length = 0)+
  geom_signif(y_position = 3000, xmin = 1.0, xmax = 3.0, annotations = "ns",
              tip_length = 0)+
  geom_signif(y_position = 4000, xmin = 2.0, xmax = 4.0, annotations = "ns",
              tip_length = 0)+
  labs(subtitle = overall_test_label,
       caption = get_pwc_label(example_of_pwc))
