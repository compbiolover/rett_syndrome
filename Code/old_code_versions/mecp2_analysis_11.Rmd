...
total_plot <- ggplot(aes(x=Combo,y=Mean, fill=Condition), data = mecp2_6_12_neg)+
  ...
  xlab("")+
  geom_sina(seed=123)+
  ggtitle("Non-PV Nuclei using PNN-neg cell type")+
  geom_signif(y_negition = c(1800, 1800), xmin = c(1.0,3.0), xmax = c(2.0,4.0),
              annotations = c("*", "ns"), tip_length = 0)+
  geom_signif(y_negition = 2200, xmin = 1.0, xmax = 3.0, annotations = "*",
              tip_length = 0)+
  geom_signif(y_negition = 2500, xmin = 2.0, xmax = 4.0, annotations = "****",
              tip_length = 0)+
  labs(
    subtitle = overall_test_label,
    caption = get_pwc_label(example_of_pwc)
...
Combining the plots
```{r, echo=FALSE}
combo_plot <- ggarrange(total_plot_neg, total_plot_pos, nrow = 1, ncol = 2, labels = c("e", "f"))
combo_plot
```
...
Building the lme for PV nuclei because of high ICC for `Cohort`
```{r, echo=FALSE}
