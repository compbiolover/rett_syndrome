...
mecp2_data_processed_6wk_simp_neg <- filter(mecp2_data_processed_6wk_simp, Cell_type=="PNN-pos")
mecp2_data_processed_6wk_simp_pos <- filter(mecp2_data_processed_6wk_simp, Cell_type=="PNN-neg")
...
mecp2_data_processed_12wk_simp_neg <- filter(mecp2_data_processed_12wk_simp, Cell_type=="PNN-pos")
mecp2_data_processed_12wk_simp_pos <- filter(mecp2_data_processed_12wk_simp, Cell_type=="PNN-neg")
...
Now doing all the statistical analysis and plotting for the non-PV Nuclei containing samples
```{r, echo=FALSE}
...
#         file = "~/Documents/PhD Program/Hong Lab/Projects/Krishnan-lab-Analysis/Data/mecp2_6_12_neg.rds")
...
twelve_wk_comp <- mecp2_6_12_neg %>% group_by(Time) %>% filter(Time=="12 wk") %>%
  dunn_test(formula = Mean~Condition, detailed = TRUE, p.adjust.method = "none")
...
overall_test_label <- create_test_label(description = "Kruskal-Wallis",
                                        ...
                                        statistic = 24.23, p = "<0.0001",
                                        parameter = "1", n = 225,
                                        effect.size.text = quote(italic(eta)^2),
                                        effect.size = 0.104, detailed = TRUE,
                                        type = "expression")
...
#Now building an LME for non-PV because of high ICC of Cohort. This is the 12 
#wk Het vs. Het model
...
#LRT test
lme_mod_base <- lme(Mean~1, data=mecp2_6_12_neg , random = ~ 1|Cohort,
                    method = "ML")

anova(lme_mod_base,lme_mod)

#Plot
total_plot <- ggplot(aes(x=Combo, y=Mean, fill=Condition),
                     data = mecp2_6_12_neg)+
  geom_violin() +
  geom_sina(seed=123)+
  theme(panel.background = element_blank(),
  ...
  geom_signif(y_position = 2000, xmin = 1.0, xmax = 3.0, annotations = "ns",
              tip_length = 0)+
  geom_signif(y_position = 2500, xmin = 2.0, xmax = 4.0, annotations = "ns",
              tip_length = 0)+
...
```{r, echo=FALSE}
ggsave(filename = "~/Documents/PhD Program/Hong Lab/Projects/Krishnan-lab-Analysis/Figures/Mecp2_figures/lme_non_pv_plot.png", device = "png", dpi = 300, units = "cm", height = 32, width = 32, plot = total_plot_pv)
```
...
```{r, echo=FALSE}
#Took the total number of neurons from mecp2_6_12_neg (het v. het comparison at 6 and 12 weeks) (number of rows) and divided it by the number of cohorts (6) because that is the variable that has an ICC of ~0.5
M_het <- 225/6

deff <- 1 + 0.5024248*(M_het)

neff <- 225/deff

analysis_df <- data.frame(M=M_het, Design_effect=deff, Effective_size=neff)
colnames(analysis_df) <- c("M", "Design effect", "Effective size")
analysis_df_gt <- gt(analysis_df)

analysis_df_gt
```
Our results show that about 11.34 cohorts is what we would need to get a sample size that would be equivalent to a sample size that had no correlation/clustering. This is about 89% more cohorts

## Overall Conclusion
The `Cohort` variable has moderate ICC (0.50) in the non-PV nuclei samples. The ICC in the PV nuclei samples was low (0.10). Given the moderate level of ICC in the non-PV nuclei a linear mixed effects model was built. This did all of the same comparisons as the standard model but taking into account the effect of the data dependence of `Cohort`. The results largely remain the same with 3/4 comparisons remaining significant and in-line with the results seen in the pre-print. The only change was the Het v. Het comparison went from being very significant to not significant. We also saw a drop in significance in the 6 week WT v. Het comparison but this is not unexpected as taking into account data dependence of variables sometimes lowers the statistical significance of a studied relationship. To get the un-correlated relationship seen between Het populations across time points we would need approximately 5.5 more cohorts