...
for (s in unique(all_plot_data$subregion)[7]) {
  ...
  # Heatmap for Jacob's new data (all cohorts combined in each subregion)
  p1 <- Heatmap(
  ...
  # PNG file format
  png(file = paste0("~/Desktop/test_heatmaps/PNG/subregion_", s, "_complex_heatmap_", my_mean, "_combined_hemisphere_jacob_data.png"), units = "in", width = 6, height = 6, res = 600)
  ...
  # SVG file format
  svg(file = paste0("~/Desktop/test_heatmaps/SVG/subregion_", s, "_complex_heatmap_", my_mean, "_combined_hemisphere_jacob_data_cohort.svg"), width = 6, height = 6)
  p1 <- draw(p1, height = unit(1, "cm") * 6)
  draw(p1, height = unit(1, "cm") * 6)
  dev.off()
}
